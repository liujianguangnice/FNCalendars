...
final class Util {
     ...
     * 得到下个月的天数
     * */
    public static int getNextMonthDayCount()
    {
        Calendar a = Calendar.getInstance();
        //System.out.println("当前月份"+(a.get(Calendar.MONTH)+1));
        a.set(Calendar.MONTH, (a.get(Calendar.MONTH)+1));
        //System.out.println("下个月份"+(a.get(Calendar.MONTH)+1));
     ...
     * 取得当月剩余天数
     * */
    public static int getCurrentMonthSurplusDay()
    {
        Calendar a = Calendar.getInstance();
        int nowDay = a.get(Calendar.DATE);
        a.set(Calendar.DATE, 1);//把日期设置为当月第一天
        a.roll(Calendar.DATE, -1);//日期回滚一天，也就是最后一天
        int maxDate = a.get(Calendar.DATE);

        return maxDate-nowDay;
    }

    /**
     * 取得天数：当月剩余天数+下个月天数
     * */
    public static int getCurrentMonthAndNextMonthDay()
    {
      /*  System.out.println("当月天数："+getCurrentMonthLastDay());
        System.out.println("当月剩余天数："+getCurrentMonthSurplusDay());
        System.out.println("下个月天数："+getNextMonthDayCount());
        System.out.println("当月剩余天数+下个月天数："+getCurrentMonthAndNextMonthDay());*/
        return getCurrentMonthSurplusDay()+getNextMonthDayCount();
    }

    /**
     * 返回今天日期,前一天.
     */
    public static int[] getTodayDateBeforeOneDay() {
        Calendar calendar = Calendar.getInstance();

        int[] todayDate = new int[3];
        todayDate[0] = calendar.get(Calendar.YEAR);
        todayDate[1] = calendar.get(Calendar.MONTH) + 1;
        todayDate[2] = calendar.get(Calendar.DATE)-1;
     ...
     * 计算日期.
     */
    public static int[] addMonth(int[] date, int add) {
        Calendar calendar = Calendar.getInstance();
        calendar.set(date[0], date[1] - 1, date[2]);
    ...
    private Util(Context context) {
        ...
        special_count =Util.getCurrentMonthAndNextMonthDay();
